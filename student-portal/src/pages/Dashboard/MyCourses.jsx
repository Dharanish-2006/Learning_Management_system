export default function MyCourses () {
  ...
  useEffect(() => {
    dashboardService.getMyCourses().then(data => { setCourses(data); setLoading(false); });
  }, []);
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {courses.map(c => (
            <div key={c.id} className="card flex flex-col">
              <div className="h-36 rounded-xl flex items-center justify-center text-5xl mb-4"
              ...
              <div className="mt-auto flex gap-2">
                <Link to={`/courses/${c.id}`} className="btn-primary btn-sm flex-1 justify-center text-xs">
